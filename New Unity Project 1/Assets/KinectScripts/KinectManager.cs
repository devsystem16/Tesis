...
//				break;
	...
	void Awake()
	{
		...
		//GameObject obj = GameObject.Find ("nivel");
		//string data = obj.GetComponent<Text> ().text;
		...
		try
		{
			...
			if (hr != 0)
			{
				Debug.Log("No se puede inicializar datos de esqueleto");
		...
		catch (Exception e)
		{
			...
			Debug.LogError("Kinect no detectado, por favor conectarlo.");
			Debug.LogError(e.ToString());
			if(CalibrationText != null)
				CalibrationText.GetComponent<GUIText>().text = message;
		...
		if(CalibrationText != null)
		{
			CalibrationText.GetComponent<GUIText>().text = "Esperando un usuario";
		}
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								if(HandCursor1 != null)
								{
									...
									HandCursor1.transform.position = Vector3.Lerp(HandCursor1.transform.position, vCursorPos,8* Time.deltaTime);
								}
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								if(HandCursor2 != null)
								{
									...
									HandCursor2.transform.position = Vector3.Lerp(HandCursor2.transform.position, vCursorPos, 10 * Time.deltaTime);
								}
