...
function Update() {
    ...
    if (cursorIsFading) {
        GetComponent.<GUIElement>().color.a = fadeValue;
...
function MoveMouse(mousePosX : float, mousePosY : float) {
    ...
    GetComponent.<GUIElement>().color.a = fadeValue = normalAlpha;
    GetComponent.<GUIElement>().texture = normalCursor;
    cursorIsFading = true;
    ...
    if (mousePosX < .005) {
        mousePosX = .005;
        GetComponent.<GUIElement>().texture = leftCursor;
    }
    if (mousePosX > .995) {
        mousePosX = .995;
        GetComponent.<GUIElement>().texture = rightCursor;
    }
    if (mousePosY < .005) {
        mousePosY = .005;
        GetComponent.<GUIElement>().texture = downCursor;
    }
    if (mousePosY > .995) {
        mousePosY = .995;
        GetComponent.<GUIElement>().texture = upCursor;
    }
